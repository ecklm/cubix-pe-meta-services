...
## Student Zones
- `local.student_zones` is built by merging JSON objects from `students/*.json`; each entry has `subdomain_name`, `name_servers`, and optional `ttl`. Missing TTL stays undefined in student JSON and resolves inside `modules/student_zone`.
- The module input `parent_zone` is structurally typed for an `azurerm_dns_zone`; pass `parent_zone = azurerm_dns_zone.cubix_root`.
- Student data is committed as one JSON file per zone under `students/`; Terraform reads these files with `fileset` and `jsondecode` to avoid concurrent PRs editing one shared file.
