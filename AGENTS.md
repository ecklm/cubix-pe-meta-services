...
## GitHub Actions / OIDC
- `.github/workflows/terraform.yml` runs `plan` on same-repo PRs through `pull_request_target`, checks out the PR head SHA, runs `apply` on pushes to `main`, and supports manual `workflow_dispatch` with `action = plan|apply`.
- PR plans run automatically for internal PRs without the GitHub Actions manual approval gate; fork PRs are skipped by the same-repo guard.
...
- `.github/workflows/register-student-zone.yml` is the Port self-service workflow: it accepts `subdomain_name` plus `name_server_1` through `name_server_4`, edits `students.auto.tfvars.json` with `jq`, and opens a PR with commit/title `user: Add <subdomain>`; the workflow automatically approves and merges the PR, then dispatches Terraform apply.
- `.github/workflows/delete-student-zone.yml` is the Port self-service workflow: it deletes the specified `subdomain_name` from `students.auto.tfvars.json` using `jq`, and opens a PR with commit/title `user: Delete <subdomain>`; the workflow automatically approves and merges the PR, then dispatches Terraform apply.
